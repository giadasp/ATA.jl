...
function _desume_pars(pars::DataFrames.DataFrame;
		model = "2PL", #1PL, 2PL, 3PL
		parametrization = "at-b") #"at-b, at-ab, at+b, at+ab")
		n_items = size(pars,1)
		if (model == "1PL")
			("b" in names(pars)) || ("d" in names(pars)) || error("difficulty parameter b or d not defined")
			c = zeros(Float64, n_items)
			a = ones(Float64, n_items)
		elseif (model == "2PL")
			("a" in names(pars) || "a1" in names(pars)) || error("discrimination parameter a1 or a not defined")
			("b" in names(pars)) || ("d" in names(pars)) || error("difficulty parameter b or d not defined")
			if "a" in names(pars)
				a = pars.a
			else
		...
		elseif (model == "3PL")
			...
			("b" in names(pars)) || ("d" in names(pars)) || error("difficulty parameter b or d not defined")
			("c" in names(pars)) || ("g" in names(pars)) || error("guessing parameter c or g not defined")
			if "a" in names(pars)
				a = pars.a
			else
				a = pars.a1
...
"""
function item_char(pars::DataFrames.DataFrame, theta::Vector{Float64};
	model = "2PL", #1PL, 2PL, 3PL
	parametrization = "at-b", #"at-b, at-ab, at+b, at+ab"
	...
	nb = size(b, 2)
	N= size(theta,1)
	p = _sig_c.([D * (a[i] * theta[n] + a2[i] * b[i, j]) for i = 1 : n_items, n = 1 : N, j = 1 : nb])
...
function student_likelihood(f::Float64, r::Vector{Float64}, i_index::Vector{Int64}, a::Vector{Float64}, b::Vector{Float64}, theta::Float64;
	logL = false)
	n_items = size(a, 1)
	likel = 0
	for i in i_index
...
- **`i::Matrix{Float64}`**: Matrix of the item information. 
"""
function resp_gen(pars::DataFrames.DataFrame, theta::Vector{Float64}, design::Matrix{Float64};
	model = "2PL", #1PL, 2PL, 3PL
	parametrization = "at-b", #"at-b, at-ab, at+b, at+ab"
	D = 1,#any number (1, 1.6)
	...
	if method == "cumulated items"
		@fastmath @inbounds  for n = 1 : N#4
			...
			StatsBase.samplei = StatsBase.sample(collect(1 : n_items), n_items, replace = false)
			i = 1#6
			while i <= n_items#7
	...
	if method == "cumulated pattern"
		for n = 1 : N#4+
			...
			for r = 1:1000
				...
				StatsBase.samplei=StatsBase.sample(collect(1 : i_index), i_index, replace=false)
				for i = 1 : i_index
					csum = p2[StatsBase.samplei[i], 1]#8
					cat = 0#9
					while csum < unif[StatsBase.samplei[i]]
