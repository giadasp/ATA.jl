...
function add_constraints!(
    ...
    if size(categorical_constraints, 1) == 0
        message[2] =
            message[2] * string("No lines in file ", constraints_file, " nothing added.\n")
    else
        ...
        for g = 1:ata_model.settings.n_groups
            if g > 1
                tests = collect(
                    ...
                    )+1):(sum(ata_model.settings.Tg[1:(g-1)])+ata_model.settings.Tg[g]),
                )
            else
                ...
                for t in tests
                    if !ismissing(categorical_constraints[con, :min])
                        ...
                        if ismissing(categorical_constraints[con, :weight])
                            ...
                            A[t][end, indices] .= -(categorical_constraints[con, :weight])
                            push!(
                                b[t],
        ...
        for t = 1:ata_model.settings.T
            for var = 1:size(ata_model.constraints[t].sum_vars, 1)
                vals =
                    copy(ata_model.settings.bank[!, ata_model.constraints[t].sum_vars[var]])
