...
function start_ATA(;
    ...
    open("OPT/Settings.jl", "w") do f
        ...
        ata_model.settings.IRT.parameters =
            DataFrames.DataFrame(ata_model.settings.bank[!, Symbol.(Inputs.IRT_parameters)])
        ...
        elseif ata_model.settings.IRT.model == "3PL"
            DataFrames.DataFrames.rename!(ata_model.settings.IRT.parameters, [:a, :b, :c]) #nqp values in interval\r\n",
        else
            push!(
                ata_model.output.infos,
