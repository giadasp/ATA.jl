...
function load_settings!(
    ...
    if !isfile(settings_file)
        ...
        open("OPT/Settings.jl", "w") do f
            ...
            ATAmodel.settings.IRT.parameters = DataFrames.DataFrame(ATAmodel.settings.bank[
                !,
                Symbol.(Inputs.IRT_parameters),
            ])
    ...
    if !isfile(constraints_file)
        ...
        if size(Categoricalconsts, 1) == 0
            ...
            for g = 1:ATAmodel.settings.n_groups
                if g > 1
                    tests = collect(
                        (sum(ATAmodel.settings.Tg[1:(g-1)])+1):(sum(ATAmodel.settings.Tg[1:(g-1)])+ATAmodel.settings.Tg[g]),
                    )
                else
            ...
            for t = 1:ATAmodel.settings.T
                for var = 1:size(ATAmodel.constraints[t].sum_vars, 1)
                    vals = copy(ATAmodel.settings.bank[
                        !,
                        ATAmodel.constraints[t].sum_vars[var],
                    ])
