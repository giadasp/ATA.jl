...
function siman!(ATAmodel::Model; starting_design=Matrix{Float64}(undef, 0, 0), start_temp=0.1, geom_temp=0.1, max_time=1000.00, results_folder="RESULTS", n_item_sample=1000, n_test_sample=2, conv_max=2, verbosity=2, opt_feas=0.0, n_fill=1 , feas_nh=0, opt_nh=5)
	...
	if size(starting_design, 1) > 0
		if (size(starting_design, 1) != n_FS || size(starting_design, 2) != ATAmodel.settings.T)
	...
	t = copy(start_temp)
	for v2 = 1:T
		NH⁺.infeas[v2], x_Iₜ = check_feas(ATAmodel.settings.FS, ATAmodel.constraints[v2], NH⁺.x[:, v2], n_FS, n_items)
	...
	for nh = 1:NHs[end]
		ATAmodel.output.neighbourhoods[nh] = _mycopy(NH⁺, ATAmodel.output.neighbourhoods[nh])
	end
	round = 1
	while finish == 0
	...
	DelimitedFiles.writedlm(string(results_folder, "/design.csv"), reshape(ATAmodel.output.design, n_FS, T))
