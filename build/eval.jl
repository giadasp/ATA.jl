function eval_overlap_v(xₜ::Vector{Float64}, x::Matrix{Float64}, FS_counts::Vector{Int64}, ol_maxₜ::Vector{Float64}, v::Int64)
	xₜ = xₜ .* FS_counts
...
function eval_Exp_Scoreₜ(x_Iₜ::Vector{Float64}, expected_scoreₜ::ExpectedScore) #x = I
	es = _gemvblas(expected_scoreₜ.val, x_Iₜ, zeros(size(expected_scoreₜ.val,1)),size(x_Iₜ, 1)) / sum(x_Iₜ)
...
function check_feas(FS::FS, Consₜ::Constraint, xₜ::Vector{Float64}, n_FS::Int64, n_items::Int64)
	es = 0
	cons = 0
...
#warmup feasibility
function fill_up_feas(NH::Neighbourhood, v::Int64, IU::IU, FS::FS, constraints::Constraint, x_forced0ₜ::Vector{Bool}, n_items::Int64, n_FS::Int64, ol_maxₜ::Vector{Float64})
	f₀, infeas₀ = Inf, Inf
	T = size(NH.x, 2)
	x₀ = copy(NH.x)
	xₜ = x₀[:, v]
...
#warmup MAXIMIN
function fill_up_MAXIMIN(NH::Neighbourhood, IIFₜ::Matrix{Float64}, opt_feas::Float64, v::Int64, IU::IU, FS::FS, constraints::Constraint, x_forced0ₜ::Vector{Bool}, n_items::Int64, n_FS::Int64, ol_maxₜ::Vector{Float64})
...
#warmup MAXIMIN CC
function fill_up_CC(NH::Neighbourhood, obj::Obj, IIFₜ::Array{Float64, 3}, opt_feas::Float64, v::Int64, IU::IU, FS::FS, constraints::Constraint, x_forced0ₜ::Vector{Bool}, n_items::Int64, n_FS::Int64, ol_maxₜ::Vector{Float64})
	f₀, TIF₀, infeas₀ = Inf, copy(NH.obj), Inf
	α = obj.aux_float
	T = size(NH.x, 2)
	x₀ = copy(NH.x)
...
#warmup custom
function fill_up_custom(NH::Neighbourhood, obj::Obj, opt_feas::Float64, v::Int64, IU::IU, FS::FS, constraints::Constraint, x_forced0ₜ::Vector{Bool}, n_items::Int64, n_FS::Int64, ol_maxₜ::Vector{Float64})
...
function analyse_NH(NH_start::Neighbourhood, ATAmodel::Model, IIF::Vector{Array{Float64, 3}}; fF = true, n_fill = 1, opt_feas = 0.9, conv_max = 1, start_temp = 1000.0, geom_temp = 0.1, n_item_sample = 1, n_test_sample = 1, verbosity = 1, start_time = 0, max_time = 1000)
			...
			while any(warmup) #filling forms
				...
				if n_t<constraints.length_max
					if opt_feas == 0 || fF == true
						NH_add = fill_up_feas(NH₁, v, ATAmodel.IU, ATAmodel.settings.FS, constraints, ATAmodel.settings.forced0[v], n_items, n_FS,  ATAmodel.settings.ol_max[:, v])
						NH_add.f = opt_feas * NH_add.f
					else
						NH_add = fill_up_CC(NH₁, ATAmodel.obj, IIF[v], opt_feas, v, ATAmodel.IU, ATAmodel.settings.FS, constraints, ATAmodel.settings.forced0[v], n_items, n_FS, ATAmodel.settings.ol_max[:, v])
					end
	...
	while coverage_ok == 0
		...
		while exit == 0 && v₂ < n_test_sample #it<size(iteratorTestItem, 1) #
			...
			while exit == 0 && h₂ < nI
				...
				while  exit == 0 && add_remove < 2
					...
					#	taken = 1
					#end
					if (add_remove==1 && sum(NH₁.x[:, v] .* ATAmodel.settings.FS.counts) >= Constraintsₜ.length_min) || (add_remove==2 && sum(NH₁.x[:, v] .* ATAmodel.settings.FS.counts) < Constraintsₜ.length_max)
						NH₁.infeas[v], x_Iₜ = check_feas(ATAmodel.settings.FS, Constraintsₜ, NH₁.x[:, v], n_FS, n_items)
						...
						if fF == false
								...
								NH₁.obj[v] = eval_TIF_CCₜ(x_Iₜ, IIFₜ; α = ATAmodel.obj.aux_float)
		...
		if sum(NH₀.infeas + NH₀.ol) + NH₀.iu <= 0 && fF == true
			fF = false
			for v = 1:T
				x_Iₜ = FS_to_items(NH₀.x[:, v], ATAmodel.settings.n_items, ATAmodel.settings.FS.items)
			...
			while any(warmup) #filling forms
				...
				if n_t<constraints.length_max
					if opt_feas == 0 || fF == true
						...
						NH_add = fill_up_MAXIMIN(NH₁, IIF[v], opt_feas, v, ATAmodel.IU, ATAmodel.settings.FS, constraints, ATAmodel.settings.forced0[v], n_items, n_FS, ATAmodel.settings.ol_max[:, v])
					end
					NH_add.ol = eval_overlap(NH_add.x, FS_counts, ATAmodel.settings.ol_max, T, NH_add.ol)
					Printf.@printf "."
					nₜ = LinearAlgebra.dot(NH_add.x[:, v], ATAmodel.settings.FS.counts)
	...
	while coverage_ok == 0
		...
		while exit == 0 && v₂ < n_test_sample #it<size(iteratorTestItem, 1) #
			...
			while exit == 0 && h₂<nI
				...
				#	taken = 1
				#end
				if sum(NH₁.x[:, v].*ATAmodel.settings.FS.counts)>= Constraintsₜ.length_min
					NH₁.infeas[v], x_Iₜ = check_feas(ATAmodel.settings.FS, Constraintsₜ, NH₁.x[:, v], n_FS, n_items)
					iu = sum(NH₁.x, dims = 2) - ATAmodel.IU.max
					iu = iu[iu .> 0]
					if size(iu, 1) == 0
...
#custom neighborhood
function analyse_NH(NH_start::Neighbourhood, ATAmodel::Model, IIF::Float64; fF = true, n_fill = 1, opt_feas = 0.9, conv_max = 1, start_temp = 1000.0, geom_temp = 0.1, n_item_sample = 1, n_test_sample = 1, verbosity = 1, start_time = 0, max_time = 1000)
	if fF == true
		NH_start.obj = zeros(Float64, ATAmodel.settings.T)
	end
	NH₁ = Neighbourhood()
			...
			while any(warmup) #filling forms
				...
				if n_t<constraints.length_max
					if opt_feas == 0 || fF == true
						...
						NH_add = fill_up_custom(NH₁, ATAmodel.obj, opt_feas, v, ATAmodel.IU, ATAmodel.settings.FS, constraints, ATAmodel.settings.forced0[v], n_items, n_FS, ATAmodel.settings.ol_max[:, v])
					end
					NH_add.ol = eval_overlap(NH_add.x, FS_counts, ATAmodel.settings.ol_max, T, NH_add.ol)
					Printf.@printf "."
					nₜ = LinearAlgebra.dot(NH_add.x[:, v], ATAmodel.settings.FS.counts)
