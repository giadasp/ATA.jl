...
function item_char(pars::DataFrames.DataFrame, theta;
	...
	p = _sig_c.([D * (a[i] * theta[k] + a2[i] * b[i, j]) for i = 1 : I, k = 1 : K, j = 1 : nb])
...
function item_info(pars::DataFrames.DataFrame, theta;
	...
	if (model == "1PL")
		...
		pars.a = ones(Float64, n_items)
	elseif (model == "2PL")
		("a1" in names(pars) || "a" in names(pars)) || error("discrimination parameter a1 or a not defined")
		("b" in names(pars)) || ("d" in names(pars)) || error("difficulty parameter b or d not defined")
		pars.c = zeros(Float64, n_items)
	elseif (model == "3PL")
		("a1" in names(pars) || "a" in names(pars)) || error("discrimination parameter a1 not defined")
...
function student_likelihood(f::Float64, r::Vector{Float64}, i_index::Vector{Int64}, a::Vector{Float64}, b::Vector{Float64}, θ::Float64;
	logL = false)
	I = size(a, 1)
	likel = 0
	for i in i_index
...
function resp_gen(f::Vector{Float64}, pars::DataFrames.DataFrame, θ::Vector{Float64}, design::Matrix{Float64};
	...
	I = size(b, 1)
	N = size(θ, 1)
	n_index = Array{Array{Int64, 1}, 1}(undef, I)
