...
#filling OptFeas==0
function WarmUp(x::Matrix{Float64},cons::Vector{Float64},v::Int64,IU::IU,FS::FS,Constraints::Constraint,x_forced0_v::Vector{Bool},nItems::Int64,nFS::Int64,FScounts::Matrix{Float64},olMax::Matrix{Float64},ol::Vector{Float64})
	f_old,  infeas_old, ol_old=  Inf, copy(cons), copy(ol)
	...
	for i2 in idx_v2
		if x_forced0_v[i2]
			...
			if (f_new<f_old)
				f_old, infeas_old , ol_old, iu_opt =  copy(f_new),  copy(infeas_new), copy(ol_new), copy(iu)
				i_best=copy(i2)
			end
		end
	end
...
function WarmUp(x::Matrix{Float64},cons::Vector{Float64},IIFv::Matrix{Float64},TIF::Vector{Float64},OptFeas::Float64,v::Int64,IU::IU,FS::FS,Constraints::Constraint,x_forced0_v::Vector{Bool},nItems::Int64,nFS::Int64,FScounts::Matrix{Float64},olMax::Matrix{Float64},ol::Vector{Float64})
	f_old, TIF_old, infeas_old, ol_old=Inf,copy(TIF),copy(cons), copy(ol)
	T=size(Constraints.ol_max,1)+1
	x_v=x[:,v]
	idx_v2=setdiff(collect(1:nFS),findall(x_v.==one(Float64))) #i2=1,...,I
	idx_v2=Random.shuffle!(idx_v2)
	...
	for i2 in idx_v2
		if x_forced0_v[i2]
			...
			if (f_new<f_old)
				i_best=copy(i2)
				f_old, TIF_old, infeas_old, ol_old, iu_opt =  copy(f_new), copy(TIF_new), copy(infeas_new), copy(ol_new), copy(iu)
			end
		end
	end
	x[i_best,v]=one(Float64)
...
#filling opt>0 CC
function WarmUp(x::Matrix{Float64},cons::Vector{Float64},IIFv::Array{Float64,3},TIF::Vector{Float64},α::Float64,OptFeas::Float64,v::Int64,IU::IU,FS::FS,Constraints::Constraint,x_forced0_v::Vector{Bool},nItems::Int64,nFS::Int64,FScounts::Matrix{Float64},olMax::Matrix{Float64},ol::Vector{Float64})
	f_old, TIF_old, infeas_old, ol_old=Inf,copy(TIF),copy(cons), copy(ol)
	T=size(Constraints.ol_max,1)+1
	x_v=x[:,v]
	idx_v2=setdiff(collect(1:nFS),findall(x_v.==one(Float64))) #i2=1,...,I
...
# function optimize(ATAmodel::model,x₀::Matrix{Float64},t₀::Float64,geom_temp::Float64;max_evals=1e6,max_time=1000.00,nItemSample=1000,nTestSample=2,convMax=1,verbosity=2,GroupByFS=false,OptFeas=0.0,nRand=1 ,feasNH=0,optNH=5)
# 	if !("RESULTS" in readdir())
# 		mkdir("RESULTS")
# 	end
# 	if ATAmodel.Settings.OptType=="MAXIMIN"
...
# 	if size(x₀,1)==0
# 		x₀=zeros(ATAmodel.Settings.nFS*ATAmodel.Settings.T)
# 	end
# 	ub=ones(ATAmodel.Settings.nFS*ATAmodel.Settings.T)
# 	lb=zeros(ATAmodel.Settings.nFS*ATAmodel.Settings.T)
# 		NH=Neighbourhood(reshape(x₀,ATAmodel.Settings.nFS,ATAmodel.Settings.T),Inf,zeros(T),1e6*ones(T),1e6*ones(T),zero(Float64))
# 	if size(x₀,1)==0
# 		NH.x=zeros(Float64,ATAmodel.Settings.nFS,ATAmodel.Settings.T)
# 	end
# 	#compute f
...
# 	f₀=copy(NH.f)
# 	x=copy(x⁺)
# 	ATAmodel.Output.Neighbourhoods=[Neighbourhood() for n=1:1]
...
# 				if (lb[i] != ub[i])
# 					ratio = nacp[i] / ns
# 					if(ratio > 0.6) bounds[i] = 1 * (1.0 + 2.0 * (ratio - 0.6) / 0.4) end
# 					if(ratio < .4) bounds[i] = 1 / (1.0 + 2.0 * ((0.4 - ratio) / 0.4)) end
...
# 			println("current best function value: ", round(f⁺, digits=5))
