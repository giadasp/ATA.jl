function jumpATA!(ATAmodel::model; starting_design=Matrix{Float64}(undef,0,0), optimizer_constructor = Cbc.Optimizer, optimizer_attributes = [("seconds",1000),("logLevel ",1)], results_folder= "RESULTS")
	message = ""
...
for t=1:ATAmodel.Settings.T
	if size(ICF_new[t],1) > 0
		for k=1:size(ICF_new[t],1)
			if ATAmodel.Constraints[t].ExS.Min[k] > 0
				JuMP.@constraint(m, sum(x[i,t]*round(ATAmodel.Constraints[t].ExS.Val[k,i];digits = 3) for i=1:ATAmodel.Settings.nFS) >= round(ATAmodel.Constraints[t].ExS.Main[k]*ATAmodel.Constraints[t].length_min;digits = 3))
				c+=1
			end
			if ATAmodel.Constraints[t].ExS.Max[k] < 1
				JuMP.@constraint(m, sum(x[i,t]*round(ATAmodel.Constraints[t].ExS.Val[k,i];digits = 3) for i=1:ATAmodel.Settings.nFS)  <= round(ATAmodel.Constraints[t].ExS.Max[k]*ATAmodel.Constraints[t].length_max;digits = 3))
				c+=1
			end
		end
	end
...
for t=1:ATAmodel.Settings.T
	if size(ATAmodel.Constraints[t].catConstrA,1)>0
		for constr=1:size(ATAmodel.Constraints[t].catConstrA,1)
			JuMP.@constraint(m, sum(x[i,t]*ATAmodel.Constraints[t].catConstrA[constr,i] for i=1:ATAmodel.Settings.nFS)<=ATAmodel.Constraints[t].catConstrb[constr]+z[c])
			c+=1
		end
	end
end
